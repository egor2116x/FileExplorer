...
public:
    ...
    const std::wstring & GetRootFolderPath() const noexcept { return m_rootFolderPath; }
	void SetRootFolderPath(const std::wstring & folderPath) { m_rootFolderPath = folderPath; }
	template<typename Output_Type> void print(Output_Type & out);
    template<typename Action> void makeAction(Action & action);
private:
	BOOL IsFile(DWORD fileAttribute) const noexcept { return !(fileAttribute & FILE_ATTRIBUTE_DIRECTORY); }
	BOOL IsDirectory(DWORD fileAttribute) const noexcept { return (fileAttribute & FILE_ATTRIBUTE_DIRECTORY); }
	void BuildNewFolderPath(std::wstring & folderPath, const std::wstring & newFolderName);
	size_t GetFileSize(WIN32_FIND_DATA & _wfd) const noexcept;
private:	
    size_t m_level;
